...
def invoice_jpg(invoice_id):
    ...
    with sync_playwright() as pw:
        ...
        invoice = page.locator(".paper-invoice, .invoice-paper, #invoice, .invoice, .toucan-invoice").first
        if invoice.count() > 0:
            invoice.screenshot(path=str(output_path), type="jpeg", quality=95)
        else:
            page.screenshot(path=str(output_path), type="jpeg", quality=95, full_page=True)
