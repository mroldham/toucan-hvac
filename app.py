...
@app.route("/filter-orders/<int:order_id>/pay")
@login_required
def filter_order_pay(order_id):
    upgrade_filter_order_workflow()
    ...
    checkout_session = stripe.checkout.Session.create(
        ...
        metadata={
            "order_id": str(order_id),
            "filter_size": order["filter_size"] or "",
        ...
        success_url=domain + "/filter-payment-success?order_id=" + str(order_id),
        cancel_url=domain + "/service-requests",
    )
...
def vip_customer_summary(customer):
    ...
    filter_monthly = float(getattr(customer, "filter_monthly_price", 0) or 0)
    filter_annual = filter_monthly * 12 if getattr(customer, "filter_club_member", False) else 0

    years_active = 0
    try:
        if getattr(customer, "customer_since", None):
            years_active = max(0, (datetime.utcnow() - customer.customer_since).days / 365)
    except Exception:
        pass
